...
def run_generate_documentation_for_repo(cloned_repo_root_path: str):
  """
  Generates overall documentation for an entire repository using the multi-agent system.
  Args:
      cloned_repo_root_path (str): The absolute path to the locally cloned repository root.
  """
  start_time = time.time()

  # Set ROOT_FOLDER environment variable for agents
  os.environ["ROOT_FOLDER"] = cloned_repo_root_path

  print(f"Generating overall documentation for repository: {cloned_repo_root_path} using multi-agent system...")
  
  overall_documentation_content = None
  wait_times = [60, 120, 240, 480, 960, 1920]
  for i, wait_time in enumerate(wait_times):
      try:
          overall_documentation_content = mac.generate_overall_repo_documentation(cloned_repo_root_path)
          break  # Success
          ...
          print(f"An unexpected error occurred during documentation generation: {e}")
          if i < len(wait_times) - 1:
              print(f"Retrying in {int(wait_time / 60)} minute(s)...")
              time.sleep(wait_time)
          else:
              raise e

  if not overall_documentation_content or "failed to run" in overall_documentation_content:
      print(f"Error: Overall repository documentation generation failed for {cloned_repo_root_path}")
      print("Agent returned:", overall_documentation_content)
      return

  # Save the overall documentation to a single markdown file in the project's docs_output folder
  repo_name = os.path.basename(cloned_repo_root_path)
  output_md_path = os.path.join(_docs_output_dir, f"{repo_name}_repo_documentation.md")
  with open(output_md_path, 'w', encoding='utf-8') as f:
      f.write(overall_documentation_content)
  
  total = round(time.time() - start_time, 3)
  print(f"\nOverall documentation generation completed in {total}s.")
  print(f"Overall repository documentation saved to: {output_md_path}")
